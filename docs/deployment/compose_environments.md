...
| `docker-compose.staging.yml` | temporary staging stack for high-risk local rebuild verification | 5434 / 8002 / 3003 | `postgres_staging_data` | yes |
| `docker-compose.staging.release.yml` | temporary staging stack for release-image verification | 5434 / 8002 / 3003 | `postgres_staging_data` | yes |
...
- `make staging-*` only when you need a short-lived local rebuild verification environment
- `make staging-release-up VERSION=x.y.z` when staging must validate the same app/frontend image tags as production
