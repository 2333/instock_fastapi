...
`M2` 对应 `P3-01` 用户行为埋点。该里程碑已于 `2026-04-20` 完成首波验收，当前不再作为 active execution pack 继续扩写；后续若需要恢复上下文，应先看验收记录，再回到 [docs/EXECUTION_PLAN.md](../../EXECUTION_PLAN.md) 确认现行路线。
...
- [../m3/PRE_M3_MAINLINE_ALIGNMENT_PLAN.md](../m3/PRE_M3_MAINLINE_ALIGNMENT_PLAN.md)：`M2` 之后的 `Pre-M3` closed baseline

当前使用约定:

- `M2` 已达成“可以验收”，不要再把仓库当前状态写成 `M2 kickoff 前`
- 如果问题涉及生产 500、schema drift、主线残留或发布门禁，不在 `M2` 目录内补锅，先回到 `docs/EXECUTION_PLAN.md` 判断应进入哪个现行执行包
- 首波只覆盖认证用户，不在本轮引入访客 / session 级事件
- 首波事件只允许白名单字段，不接收任意 shape 的 `event_data`
- 写入失败不得影响业务主流程，但失败事实必须可观测、可留痕
- `M2` 已通过 reviewer gate；`Pre-M3` 与 `M3` 均已作为后续 closed baseline 保留
