...
> 历史说明: 本文档记录的是 `M0 rebaseline` 时期的治理方案。其上下文和约束用于解释为什么仓库先经历 rebaseline，再进入 `M1` 与后续里程碑。
>
> 当前使用约定:
> - 本文档保留为治理背景与决策依据，不作为当前开发阶段的直接执行入口
> - 当前主路线与下一阶段推进顺序，以 `docs/EXECUTION_PLAN.md` 为准
>
> 版本: 2026-04-06
> 原状态: R4 review
> 原目标: 将 `origin/main` 收回为“真正的 M0 基线”，随后再从干净主线启动 `M1`
