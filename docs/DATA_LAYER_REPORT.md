...
```
          ...
          │  stocks表补全        │ │  daily_basic       │ │  跨源校验           │
          │  moneyflow替换      │ │  stock_st          │ │  数据完整性检查      │
          │  top_list替换       │ │  broker_forecast   │ │  告警与监控         │
...
### 4.2 WS-0: 基础设施层 (前置条件, 约2天)

> **目标**: 为后续所有工作流提供 DB迁移、通用行情入口、数据质量框架

| Task ID | 任务 | 交付物 | 依赖 |
|---------|------|--------|------|
| WS0-01 | 引入 Alembic 迁移框架 | `alembic/`, `alembic.ini`, 首次 auto-generate | 无 |
| WS0-02 | 实现通用行情抽象 `TushareProvider.fetch_pro_bar()` | 支持 asset=E/I/FT/O/FD | 无 |
| WS0-03 | 数据质量基础框架 | `core/quality/validator.py`: 空值率检查、数值范围校验、行数预期校验 | 无 |
| WS0-04 | 积分检测工具 | `scripts/check_tushare_points.py`: 检查当前积分，输出可用接口列表 | 无 |

**WS0-01 ~ WS0-04 可并行开发。**

### 4.3 WS-1: 核心数据改造 (约3天)

> **目标**: 改造现有表结构，统一行情获取路径，保留全部字段

| Task ID | 任务 | 交付物 | 依赖 |
|---------|------|--------|------|
| WS1-01 | `stocks` 表补全字段 | Alembic migration + model更新 + `stock_basic` 全字段入库 | WS0-01 |
| WS1-02 | `fund_flows` → `moneyflow` 表替换 | 新表 DDL + 数据迁移脚本 + `moneyflow` 接口对接 | WS0-01 |
| WS1-03 | `stock_tops` → `top_list` 表替换 | 新表 DDL + `top_list` 接口全字段对接 | WS0-01 |
| WS1-04 | `daily_bars` 统一走 `pro_bar` | 重构 `fetch_daily_task.py` 使用 `fetch_pro_bar(asset="E")` | WS0-02 |
| WS1-05 | 指数数据入库 | `pro_bar(asset="I")` + daily_bars 表 asset_type 字段 | WS0-02, WS1-04 |
| WS1-06 | 更新市场下游服务 | `market_data_service.py`, `fund_flow_service.py` 适配新表 | WS1-02, WS1-03 |

**WS1-01, WS1-02, WS1-03 可并行 (均只依赖 WS0-01)。**
**WS1-04 依赖 WS0-02, WS1-05 串行在 WS1-04 之后。**
...
**WS2-01 ~ WS2-06 全部可并行开发，互不依赖。** 这是并行度最高的工作流。

### 4.5 WS-3: 数据质量保障 (约2天)

> **目标**: 建立数据质量检查、跨源校验、告警机制

| Task ID | 任务 | 交付物 | 依赖 |
|---------|------|--------|------|
| WS3-01 | 数据完整性检查 | 检查每日各表数据行数是否符合预期 (股票数 vs 实际入库数) | WS0-03 |
| WS3-02 | 数值范围校验 | 价格>=0, 涨跌幅 [-20%, +20%] (含北交所30%), 成交量>=0 | WS0-03 |
| WS3-03 | 跨源一致性校验 | Tushare vs EastMoney 收盘价/成交量抽样比对 | WS0-03 |
| WS3-04 | 数据缺失告警 | 交易日无数据时发送告警 (log + 可选webhook) | WS3-01 |
| WS3-05 | 历史数据回填工具 | `scripts/backfill.py`: 按日期范围+接口批量回填 | WS0-01 |

**WS3-01 ~ WS3-03 可并行。WS3-04 基于 WS3-01。WS3-05 独立。**
...
| 角色 | 职责 | 任务分配 |
|------|------|----------|
| **Agent A (基础架构)** | DB迁移 + 质量框架 | WS0-01, WS0-03, WS0-04 → WS3-01~WS3-05 |
| **Agent B (行情统一)** | 通用行情 + 核心改造 | WS0-02 → WS1-04, WS1-05, WS1-06 |
| **Agent C (表结构改造)** | 现有表重构 | WS1-01, WS1-02, WS1-03 (WS0-01完成后立即启动) |
| **Agent D (新接口1)** | daily_basic + moneyflow + top_list | WS2-01, WS2-02, WS2-03 |
| **Agent E (新接口2)** | 筹码 + 技术因子 | WS2-04, WS2-05, WS2-06 |
| **人工 (架构决策)** | Review + 积分确认 + 生产部署 | PR Review, WS0-04 结果确认, 调度参数调优 |

### 时间线 (理想并行)

```
Day 1:  ║ WS0-01 ║ WS0-02 ║ WS0-03 ║ WS0-04 ║           ← 全并行
        ╠════════╩════════╩════════╩════════╣
Day 2:  ║ WS1-01 ║ WS1-02 ║ WS1-03 ║ WS2-01~06 (启动)    ← WS1 + WS2 并行
Day 3:  ║ WS1-04  WS1-05 ║ WS2-01~06 (继续) ║ WS3-01~03  ← 三路并行
Day 4:  ║ WS1-06 ║ WS2 收尾 ║ WS3-04,05 ║                 ← 收尾
Day 5:  ║ WS4-01~06 ║ 集成测试 ║                           ← 服务层
Day 6:  ║ E2E 测试 ║ 文档更新 ║ 部署 ║                     ← 交付
```
...
| 风险 | 概率 | 影响 | 缓解 |
|------|------|------|------|
| Tushare 积分不足 (筹码、技术因子需5000+) | 高 | WS2-04~06 无法接入 | WS0-04 首先检查积分，不足则调整优先级 |
| 大表迁移影响已有数据 | 中 | fund_flows → moneyflow 迁移可能丢数据 | 写迁移脚本时保留旧表作备份 |
| 接口频率限制导致获取慢 | 中 | 全市场筹码数据量大 (5000股 × 每股一次请求) | 分批获取 + 合理sleep + 按活跃度优先 |
| stk_factor_pro 字段过多 (210+) | 低 | 表宽度大，查询性能可能下降 | 考虑列存储或仅存常用因子子集 |
| 现有服务中断 | 中 | 表结构改造期间本地开发测试中断 | 使用 Alembic 做可回滚迁移，改造在独立分支进行 |
