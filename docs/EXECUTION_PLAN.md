...
- 当前日常执行只认 3 份 route docs：`EXECUTION_PLAN.md`、`docs/milestones/m5/README.md`、`docs/milestones/m5/M5_P3-05_KICKOFF_PLAN.md`。
...
- 当前最高优先级是启动 `M5 / P3-05`，先冻结参数优化服务的最小闭环、任务执行模型、schema/API contract 和回滚边界。
...
2. 再看 `docs/milestones/m5/README.md`，确认当前 `M5 / P3-05` 的启动边界、依赖顺序和 residue 决策。
3. 再看 `docs/milestones/m5/M5_P3-05_KICKOFF_PLAN.md`，确认并行任务拆分、stop condition、验证和回滚边界。
4. 需要设计输入时，再看 `docs/milestones/phase3/PHASE3_P3-05_PARAM_OPTIMIZATION*.md`。
5. 如果要回看 `Pre-M3` / `M3` 的关闭证据，再看 `docs/milestones/m3/`。
6. 只有需要证据、命令、变更清单和中断恢复细节时，才进入各 wave artifact。
...
**第一优先级: 启动 `M5 / P3-05` 参数优化**
1. 冻结 `M5` 启动边界：只把参数优化任务、trial 结果、进度、最优参数应用纳入首轮最小闭环。
2. 统一文档口径：`EXECUTION_PLAN.md`、`docs/README.md`、`docs/milestones/README.md`、`docs/milestones/m5/README.md` 必须一致描述当前 active 为 `M5 / P3-05`。
3. 处理参数优化残片：`app/optimization/algorithms.py` 先标记为 experimental residue，不视为 runtime-ready；前端 optimization CSS 残留不代表页面或 API 已存在。
4. 保持 `schema_contract` 发布 gate 生效，`M5` 的 job/trial schema 变更继续走 truthfully managed release path。
...
- `M2`、`Pre-M3` 与 `M3 / P3-03` 均已完成；当前下一步是启动 `M5 / P3-05` 参数优化
...
- [ ] **P3-05 参数优化服务**（当前主线；执行入口见 `docs/milestones/m5/README.md`）
  ...
  - 并发控制、取消、回滚边界与 schema gate 必须在 kickoff 中冻结
