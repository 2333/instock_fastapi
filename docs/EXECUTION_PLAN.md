...
2. 再看 `docs/milestones/m3/M3_P3-03_ALERT_ENGINE_PLAN.md`，确认当前 `M3` 的锚定目标、canonical model、分步交付与 stop condition。
3. 如果要确认为什么当前可以进入 `M3`，再看 `docs/milestones/m3/PRE_M3_DECISION.md`。
4. 如果要回看 `Pre-M3` 的关闭证据，再看 `docs/milestones/m3/PRE_M3_MAINLINE_ALIGNMENT_PLAN.md` 与 `PRE_M3_MAINLINE_ALIGNMENT_TRACKER.md`。
5. 需要更细设计输入时，再看 `docs/milestones/phase3/PHASE3_P3-03_ALERT_ENGINE*.md`。
...
#### `M3` `P3-03` 参数化指标筛选与订阅提醒 Kickoff

> 当前执行入口: `docs/milestones/m3/M3_P3-03_ALERT_ENGINE_PLAN.md`
> 当前状态: `M3-A` completed; `M3-B` acceptance candidate completed; pending user acceptance / optional post-M3 entry polish
> 目标: 以 `Saved Screener + Alert Subscription` 为主模型；当前已完成 `M3-A` 的 canonical/runtime/frontend baseline 与 `M3-B` 的订阅、手动触发、盘后 checker、run/hit/notification 闭环，并已通过 reviewer closure
...
**下一步**
- `M3` acceptance：用户验收当前最小闭环（参数化筛选 -> 保存筛选器 -> 创建订阅 -> 手动触发 / scheduler checker -> notification）
- `M3-C`：作为验收后增强项，只做入口收敛，不阻塞当前 `M3` 最小可验收闭环；不提前扩散到邮件、铃铛或 Attention 快捷入口
...
**第一优先级: 收口 `M3 / P3-03` 验收与 PR**
1. 冻结本次 PR 边界，只包含 `Saved Screener + Alert Subscription + Registry + Adapter` 最小闭环、`M2` 事件底座、`Pre-M3` release/runtime/doc 对齐和必要测试/脚本。
2. 补齐最终验收证据：全量后端测试、M3 focused suite、前端 build、Alembic head/current、真实环境 smoke、scheduler readiness 行为、release precheck。
3. 统一文档口径：`EXECUTION_PLAN.md`、`docs/milestones/m3/README.md`、`docs/milestones/m3/M3_P3-03_ALERT_ENGINE_PLAN.md` 必须一致描述当前为 `M3` 最小验收候选。
4. 保持 `schema_contract` 发布 gate 生效，所有 schema-changing 变更继续走 truthfully managed release path。
...
- `M2` 与 `Pre-M3` 均已完成；当前 `M3 / P3-03` 已进入最小验收候选，下一步是验收、PR 与合并
...
3. **参数化指标筛选与订阅提醒**（当前主线）
   ...
   - 当前等待最终验收与 PR 收口；`M3-C` 快捷入口是验收后增强项
...
- [-] **P3-03 参数化指标筛选与订阅提醒**（当前主线；设计文档见 `docs/milestones/m3/M3_P3-03_ALERT_ENGINE_PLAN.md`）
  ...
  - 待完成：远端 PR 状态确认、提交 PR 与合并
- [ ] **P3-05 参数优化服务**（设计文档见 `docs/milestones/phase3/PHASE3_P3-05_PARAM_OPTIMIZATION.md`）
  - 自动化参数调优（随机搜索 + 贝叶斯优化）
  - 异步任务 + 进度查询 + 结果对比图表
  - 过拟合检测（滚动窗口验证 + 警告）
  - 并发控制 + 最优参数一键应用
