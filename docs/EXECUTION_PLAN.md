...
> 版本:v2.5 | 更新日期:2026-05-21
> 基于 PRD v2.0 / ROADMAP v2.0 + 代码实际进度重写
> v1.0 归档:[EXECUTION_PLAN_v1_archive.md](./archive/EXECUTION_PLAN_v1_archive.md)
>
> **当前状态**: `M0 rebaseline`、`M1` 当前阶段、`M2`、`Pre-M3` 与 `M3 / P3-03` 均已完成验收；`M5 / P3-05 v1` 已完成本地代码验收并已提交 PR，当前 active 是 `M5` PR/release closure
...
- 当前 active 路线是 `M5 / P3-05` PR/release closure；`Pre-M3` 与 `M3` 均作为 closed baseline 保留在 `docs/milestones/m3/`。
- 当前日常执行先认 3 类 route/evidence docs：`EXECUTION_PLAN.md`、`docs/milestones/m5/README.md`、`docs/milestones/m5/artifacts/`。`docs/milestones/m5/M5_P3-05_KICKOFF_PLAN.md` 只作为冻结 kickoff 基线和历史启动计划，不再代表当前唯一执行状态。
- `docs/milestones/m3/` 只作为 `Pre-M3` 与 `M3 / P3-03` closed baseline 回看，wave artifact 只用于证据和恢复，不承担当前路线裁决。
...
| `Pre-M3` | 主线对齐修复 | 先修发布正确性、runtime/schema 漂移、文档漂移与主线残留，再进入 `M3` |
| `M3` | `P3-03` 参数化指标筛选与订阅提醒 | `Saved Screener`、参数化指标、订阅提醒、通知闭环 |
| `M5` | `P3-05` 参数优化 | 优化任务、进度、结果对比、最佳参数应用 |
| `M6` | `P3-06` 数据洞察报告 | 报告生成、偏好设置、定时任务、通知入口 |
| `M7` | `P3-02` 推荐 | 依赖行为数据积累,作为 P3 收官 milestone |

> 说明: 旧 `M4 / P3-04 策略社交` 已于 `2026-04-20` 从 active plan-of-record 退役；编号不复用,以减少历史引用混乱。

### 当前优先级解释

- `M0` rebaseline 已完成,当前仅作为治理背景保留。
- `M1` 当前阶段已完成验收；`daily_basic` / `stock_st` follow-up 保留在 backlog 中,但默认不阻塞主线推进。
- `M2` 与 `Pre-M3` 已完成验收；`M3 / P3-03` 已完成 post-merge staging 与 `0.4.1` 生产收尾。
- `M3` 已通过 post-merge staging 验证并以 `0.4.1` 收尾；`M3-C` 快捷入口、铃铛、邮件与 Attention 快捷入口转入后续增强，不阻塞主线。
- 当前最高优先级是关闭 `M5 / P3-05` PR/release：本地代码验收与 PR 提交已完成，live staging smoke、生产 backup、发布前 schema-contract gate 与 release smoke 仍属于 release activity。
...
- 当前所在位置: `M5 / P3-05 v1` 已完成本地代码验收并已提交 PR；当前短期 active 是 `M5` PR/release closure
- 当前非阻塞 backlog: `daily_basic` / `stock_st` live gate、backfill rehearsal、质量留痕与相关筛选接入
- 当前 blocker cluster: 无 M3 收尾 blocker；`M5` 仍需 release activity 收口，不应写成生产完成
- 下一阶段目标: 完成 `M5` PR 合并与发布收口；合并/发布后再进入 `M6 / P3-06`

### 当前阅读顺序

1. 先看本文件，确认当前里程碑位置和高层 stop condition。
2. 再看 `docs/milestones/m5/README.md`，确认 `M5 / P3-05 v1` 的本地验收状态、release activity 与 residue 决策。
3. 再看 `docs/milestones/m5/artifacts/`，尤其是 `W1-backend-v1-*` 与 `W2-frontend-v1-alignment.md`，确认本地代码验收、命令证据和剩余 release activity。
4. 如需回看启动范围，再看 `docs/milestones/m5/M5_P3-05_KICKOFF_PLAN.md`；它是冻结 kickoff 基线/历史启动计划，不是当前唯一执行状态。
5. 需要设计输入时，再看 `docs/milestones/phase3/PHASE3_P3-05_PARAM_OPTIMIZATION*.md`。
6. 如果要回看 `Pre-M3` / `M3` 的关闭证据，再看 `docs/milestones/m3/`。
...
#### `2026-04-06` 决策更新：先做 `M0 rebaseline`

- `origin/main` 已合入 `a0a729f [M0] Phase 0/1 Baseline Freeze (#8)`，但经审阅确认，实际合并范围已超出 `docs/milestones/m0/M0_PR_BOUNDARY.md` 约定的 `M0` 边界。
- 因此当前不直接启动 `M1`，而是先执行一次 `rebaseline`，把 `main` 收回到“运行时行为、代码范围、文档口径”都一致的真正 `M0`。
- 执行文件入口：
  - 总方案：`docs/REBASELINE_M0_PLAN.md`
  - 实时跟踪：`docs/REBASELINE_M0_TRACKER.md`
  - 任务断点记录约定：`docs/rebaseline_artifacts/README.md`

#### `M1` Phase 1.5 数据层 / Timescale 底座

> 目标: 直接完成数据层目标态，为后续 P3 依赖调度和时序能力的功能提供稳定底座。
> 当前重启入口: `docs/milestones/m1/M1_RESTART_PLAN.md`
> 当前状态: 当前阶段已完成并可验收；`daily_basic` / `stock_st` 及其相关筛选接入转为 M1 follow-up backlog，不再阻塞 `M2`
...
| M1-04 | 核心事实表 migration | Agent A + Agent C | `daily_bars` / `fund_flows` / `indicators` / `patterns` 迁移；`moneyflow` 替换另开显式 migration | M1-02, M1-03 |
| M1-05 | `pro_bar` 与抓取任务切换 | Agent B | `TushareProvider.fetch_pro_bar()` + `fetch_daily_task` 切到 `asset="E"` | M1-02 |
| M1-06 | 新事实表接入 | Agent C + Agent D + Agent E | 当前阶段优先 `technical_factors(local)`；`daily_basic` / `stock_st` 在有效 Tushare token 或单独批准的 source contract 前保持 gated follow-up；`broker_forecast` / `chip_*` 按权限 gated | M1-01 |
...
- 至少 1 次 `daily_bars` 回填演练 + 1 次新事实表回填演练完成并留痕
- 当前阶段新事实表验收以 `technical_factors(local)` 为准；`daily_basic` / `stock_st` 不再阻塞当前阶段收口
- 能支撑 `P3-03` / `P3-05` / `P3-06` 进入开发和验收
- 旧 `M1` kickoff/readiness 文档仅作历史参考；如引用已删除脚本，以 `M1_RESTART_PLAN.md` 为准

**`M1` 当前阶段验收结论（2026-04-14）**
- 已完成并通过当前阶段验收：Alembic existing-schema 路径、`daily_bars`、`technical_factors(local)`、质量检查、Timescale 健康检查、API/read-path 回归
- `daily_bars` 已验证显式 source policy；当前默认策略为 `prefer_tushare`，保持 “Tushare 优先但不是必须”，并记录实际命中的 source
- disposable PostgreSQL/TimescaleDB 已完成 live rehearsal；现有生产库不作为本阶段验收环境
- 当前阶段结论为 `go`；后续未完成项转入 `M1 follow-up backlog`

**`M1` Follow-Up Backlog（M2 前不阻塞）**
- `daily_basic` live gate、backfill rehearsal、质量留痕
- `stock_st` live gate、backfill rehearsal、质量留痕
- 基于 `daily_basic` / `stock_st` 的筛选条件与 API 扩展
- 如后续需要，再为 `daily_basic` / `stock_st` 审批单独 source contract，或等待有效 Tushare token 后恢复 Tushare 主口径
- 这些事项属于 `M1` 补完，不改变“当前阶段 M1 已完成”的判断

**`M1` 合并边界**
- 可合并分支：`m1/data-layer-restart`
- 本次合并目标：只合并当前阶段 `M1` 已验收范围，不把 `daily_basic` / `stock_st` follow-up 重新抬回 blocker
- 合并包应包含：Alembic/Timescale 路径、`daily_bars` source policy、`technical_factors(local)`、质量与健康检查、M1 文档与验收 artifact
- 合并包不应假装包含：`daily_basic` / `stock_st` live gate 完成、相关筛选条件完整接入、Tushare 恢复后的扩展数据契约
- 合并前最终检查：后端回归、`git diff --check`、M1 artifact 链接完整、follow-up backlog 已单独列账

**`M2` 启动关系**
- `M2` 不需要等待上述 follow-up 全部完成后再开始
- `M2` 启动前只需承认：当前阶段 M1 已完成，follow-up 已单独列账并继续追踪
- 如果 `M2` 某项能力直接依赖 `daily_basic` / `stock_st`，则把对应依赖写回该任务，而不是重新打开整个 M1
...
- `M2`: `P3-01` 用户行为埋点
- `Pre-M3`: 主线对齐修复
- `M3`: `P3-03` 参数化指标筛选与订阅提醒
- `M5`: `P3-05` 参数优化
- `M6`: `P3-06` 数据洞察报告
- `M7`: `P3-02` 推荐

旧 `M4 / P3-04` 已退役,仅作为历史设计留存在 `docs/milestones/phase3/`，不再作为 active milestone。

#### `M2` `P3-01` 用户行为埋点 Kickoff

> 推荐分支: `m2/p3-01-user-behavior-tracking`
> 阶段目标: 上线最小可用行为事件采集闭环，在不影响主流程性能的前提下开始稳定积累推荐、报告、运营分析所需事件数据
> 最终目标: 连续沉淀 `14+` 天可查询、可聚合、可回放的用户行为数据，为 `M6` 报告和 `M7` 推荐提供稳定输入

**与 `M1` 的阶段边界**
- 依赖 `M1` 已接受的 Alembic existing-schema 路径，为 `user_events` 追加迁移；不重新打开 `M1` baseline 讨论
- 首波只依赖当前已稳定的认证链路和核心页面：`Dashboard`、`Selection`、`Backtest`、`StockDetail`、`Attention`
- 不依赖 `daily_basic` / `stock_st` follow-up；若后续事件增强确实需要这两类数据，单独挂回对应 `M2` 任务，而不是回退整个 `M1`

| Workstream | Owner | 范围 | 首波输出物 | 依赖 |
|------------|-------|------|------------|------|
| WS-1 Schema / Contract | Agent A + Agent B | 事件白名单、请求 schema、`user_events` 表、索引与迁移 | 事件契约冻结 + Alembic migration | 当前阶段 `M1` Alembic 路径 |
| WS-2 Service / API | Agent B | `POST /api/v1/events/track`、best-effort 写入、限流、认证用户识别 | track API + service + focused tests | WS-1 |
| WS-3 Frontend Integration | Agent C | `useAnalytics` composable、5 个核心页面接入 | 首波事件发射闭环 | WS-2 契约冻结 |
| WS-4 Tests / Ops / Docs | Agent F | SQL 验证、手工链路、性能守门、回滚边界、文档留痕 | 验收记录 + rollback note | WS-1 ~ WS-3 |

**首波任务**

| Task ID | 任务 | Owner | 输出物 | 依赖 |
|---------|------|-------|--------|------|
| M2-01 | 冻结事件白名单与隐私边界 | Agent B + Agent F | 首波事件目录、字段约束、禁止采集字段列表 | `M1` 当前阶段完成 |
| M2-02 | 新增 `user_events` 模型与 Alembic migration | Agent A | 表结构、索引、迁移验证记录 | M2-01 |
| M2-03 | 落地 `POST /api/v1/events/track` | Agent B | router/service、限流、best-effort 写入、focused tests | M2-02 |
| M2-04 | 接入前端埋点 composable 与 5 页面核心事件 | Agent C | `page_view`、`dashboard_card_click`、`filter_run`、`backtest_run`、`pattern_view`、`attention_action` | M2-03 |
| M2-05 | 完成首波验收留痕 | Agent F | SQL 样本、测试命令、手工链路、回滚边界 | M2-04 |

**首波验收标准**
- `user_events` 迁移可在当前接受的 existing-schema 起点成功执行
- `POST /api/v1/events/track` 可用；非法 payload 被拒绝；写入失败不阻塞主流程
- 5 个核心页面已接入首波 6 类核心事件
- 至少 1 条端到端手工链路可通过 SQL 查到完整事件序列
- 后端 focused tests 通过；前端 `build` / `typecheck` 通过
- 首波只要求最小可用闭环；`10+` 事件类型扩展作为 `M2` 收口前补齐项，不阻塞 kickoff

**`M2` Follow-Up（不阻塞 Kickoff）**
- 扩展到 `10+` 事件类型并补充事件版本化策略
- 增加未登录访客 / session 级事件口径
- 评估事件归档、分区或保留策略
- 如需引入 `daily_basic` / `stock_st` 衍生事件字段，单独挂回对应任务

#### `Pre-M3` 主线对齐修复

> 当前执行入口: `docs/milestones/m3/PRE_M3_DECISION.md`
> 当前状态: closed baseline; `GO to start M3`
> 目标: 在不引入兜底代码的前提下，把 `代码版本 / migration 版本 / 生产库实际 schema / 主线文档` 收拢到一致状态，并清理不属于当前 `PRD` 主线的 runtime residue
> 当前 route docs 只看 3 份: `EXECUTION_PLAN.md`、`PRE_M3_MAINLINE_ALIGNMENT_PLAN.md`、`PRE_M3_MAINLINE_ALIGNMENT_TRACKER.md`；`PRE_M3_DECISION.md` 只作为 gate file 使用，tracker 与 wave artifact 继续承担恢复辅助

| Wave | 目标 | Owner | 输出物 | Gate |
|------|------|-------|--------|------|
| `W0` 范围冻结 | 冻结主线边界、恢复顺序、责任分工与返工规则 | controller | `PRE_M3_MAINLINE_ALIGNMENT_PLAN.md`、tracker、W0 artifact | `W0` 未冻结前不允许随意并行改动；closure 以 `PRE_M3_DECISION.md` 为准 |
| `W1` 发布正确性 | 修复 release correctness，补 migration/stamp/schema gate，并把未来 schema-contract 发布所需的 truthful onboarding 路径写实 | `worker-release` | release precheck 方案、prod predeploy / postdeploy 验证路径、artifact | 没有明确 prod schema 对齐路径即 no-go |
| `W2` runtime/schema 对齐 | 清理 `Strategy` residue、修复 ORM 读路径与对应 focused tests | `worker-runtime` | model/schema/router/task 对齐、artifact | 未覆盖所有 ORM 读路径即 no-go |
| `W2a` 接口基线对齐 | 修复已知线上 `500`，并固化 representative API contract smoke | controller + `worker-runtime` | API contract 修复、smoke script、artifact | representative contract smoke 未通过即 no-go |
| `W2b` 数据完整性对齐 | 收口 active-path 的 partial-gap detection、BaoStock source-contract anomaly inventory、bounded explicit-window repair 与 fault-mode tests；历史 backfill 不属于 `Pre-M3` blocker | controller + `worker-runtime` | data completeness artifact | active-path blind spot 仍未闭环或 fault-mode tests 未通过即 no-go |
| `W3` 残留清理 | 处理导航降级、误导文案与会把偏航能力伪装成主线承诺的 residue | `worker-runtime` | residue cleanup artifact | 误删真实 contract 或仍保留误导性 active entry 即返工 |
| `W4` 文档主线终扫 | 在 `W1` ~ `W3` 后做最终 doc sweep，确保入口、状态、stop condition 与恢复路径全部写实 | controller | 文档更新、artifact | 文档与 runtime / release 结论不一致即 no-go |
| `W5` 审核与放行 | reviewer 汇总风险、给出 `M3` 是否可启动 | `reviewer` | `PRE_M3_DECISION.md` | reviewer 未给 `go` 不进入 `M3` |

**`Pre-M3` stop condition / go-to-M3 裁决**
- 关闭条件与 `go to start M3` 只以 `docs/milestones/m3/PRE_M3_DECISION.md` 为准
- 本节只负责说明“当前在哪、下一步做什么”，不再维护第二套关闭条件

#### `M3` `P3-03` 参数化指标筛选与订阅提醒 Closed Baseline

> 当前执行入口: `docs/milestones/m3/M3_P3-03_ALERT_ENGINE_PLAN.md`
> 当前状态: closed baseline；已完成 post-merge staging 验证并以 `0.4.1` 生产收尾
> 目标: 以 `Saved Screener + Alert Subscription` 为主模型；已完成 `M3-A` 的 canonical/runtime/frontend baseline 与 `M3-B` 的订阅、手动触发、盘后 checker、run/hit/notification 闭环，并已通过 reviewer closure

**M3 canonical model**
- `Saved Screener`：唯一 authored truth source
- `Alert Subscription`：只绑定筛选器版本，不复制定义
- `Registry`：唯一字段/参数/操作符/能力目录
- `Adapter`：当前默认 `SQL + TA-Lib`，未来允许切到 `vbtpro`，但不得改变 persisted definition

**首切片范围**
- 参数化 `Saved Screener` canonical envelope
- 首批 `MACD / RSI / BOLL` registry 与 metadata API
- 手动全市场筛选与结果预览
- 当前默认 `SQL + TA-Lib` adapter
- `schema_version` / `definition_version` / `definition_hash`
- legacy flat `params` 的 canonicalize + compat readback

**当前已完成**
- `Saved Screener` canonical envelope 已落到 `selection_conditions.params`
- `schema_version / definition_version / definition_hash` 已接入后端持久化与读取链路；当前前端保存 contract 是 `definition + compat params`
- `Registry` 已具备 `RSI / MACD / BOLL` 参数 schema、默认参数与 adapter 支持矩阵
- `/selection`、`/screening/run`、`/selection/today-summary` 已支持 canonical definition 驱动执行
- 默认参数保持旧 contract；非默认参数已通过 `SQL + TA-Lib` runtime 生效
- `Selection` 页面已读取 registry metadata，并暴露 `RSI period`、`MACD fast/slow/signal`、`BOLL period/stddev` 最小参数输入
- `Selection` 页面保存与运行已统一提交 `definition + compat filters`，已保存条件加载优先按 canonical definition 回填
- 本地分支后端 live smoke 已通过：`/screening/metadata`、`POST /selection/my-conditions`、`GET /selection/my-conditions`、`GET /selection/today-summary`、`POST /screening/run`
- `M3` migration lane 已收口到单一 head，`m3_alert_engine_baseline` 仅作为 legacy prototype 的 onboarding-safe 兼容基线，不再代表 `M3-B` 的主模型；`alembic upgrade head` 本地通过
- `M3-B/S1` 已新增 `alert_subscriptions / alert_runs / alert_run_hits`，并扩展 `notifications` 的 `subscription_id / alert_run_id / notification_type / dedupe_key`
- `M3-B/S1` 已支持：创建订阅、手动触发、写入 run/hit、生成站内摘要通知、同日去重、跨日新增命中判断、冷却抑制、筛选器编辑后订阅标记 `stale`
- `M3-B/S2` 已在 `Selection` 已保存条件列表提供最小订阅入口：创建订阅、查看订阅状态、手动触发并展示本次 run 摘要
- `M3-B/S3` 已新增 `app.jobs.tasks.alert_subscription_task` 并接入 scheduler：盘后仅运行 `active + post_close` 且目标交易日缺 run 的订阅，复用 `AlertSubscriptionService`；目标交易日 `daily_bars` 未就绪、active universe partial gap，或订阅使用 `pattern` rule 但缺少 `pattern_recognition` 完成审计时跳过，避免回退上一交易日或用不完整数据制造误报/假阴性
- `M3-B/S3` 已注册 `run_post_close_alerts` 调度任务，时间为策略选股之后的 `20:25`；应用启动后先执行 market recovery，再执行 alert checker，只补 latest trade date 的缺失 run，不扫历史窗口，不引入邮件、铃铛、Attention 快捷入口或历史 backfill

**后续增强**
- `M3-C` 快捷入口、铃铛、邮件与 Attention 快捷入口只作为验收后增强项，不阻塞当前主线进入 `M5 / P3-05`。
...
| MarketDataProvider 抽象层 | ✅ 历史能力 | `core/providers/market_data_provider.py`；当前 `Selection` 主路径不再依赖 provider runtime |
...
- `app/services/selection_service.py` 当前主路径已收敛为 canonical saved-screener adapter；历史 provider runtime 不再作为 `Selection` / `Screening` 执行路径
...
- **已完成** @ commit `6233d5c`
- MarketDataProvider 抽象层已引入为历史能力
- 当前 `Selection` / `Screening` 调用方向单一: router → service → canonical saved-screener adapter
...
> 决策日期:2026-04-02
> 当前重启方案:[M1_RESTART_PLAN.md](./milestones/m1/M1_RESTART_PLAN.md)
> 历史技术分析:[DATA_LAYER_REPORT.md](./milestones/m1/DATA_LAYER_REPORT.md)
...
- 核心事实表正式落地 TimescaleDB:`daily_bars`、`fund_flows`、`indicators`、`patterns` 迁移为 hypertable,并补 compression 基线；M1 不做 retention 删除
- `fund_flows` 在 M1 restart 中先作为兼容表名保留；如替换为 `moneyflow`,必须另开显式 migration 和服务切换任务
- `stock_tops` → `top_list`:对齐 Tushare `top_list` 全字段
- `daily_bars` 统一走 `TushareProvider.fetch_pro_bar(asset="E")`；指数/基金/期货等资产接入先延后,除非后续 M1 任务显式纳入
- 历史 Timescale 过渡脚本已从 active scripts 退场；后续数据层迁移由 M1 里程碑方案重新承接

**新增事实表优先级:**

| 表名 | Tushare 接口 | M1 口径 |
|------|-------------|---------|
| `daily_basic` | `daily_basic` | gated follow-up；需有效 Tushare token 或单独批准的 source contract |
| `stock_st` | `stock_st` | gated follow-up；需有效 Tushare token 或单独批准的 source contract |
| `technical_factors` | `stk_factor_pro` | 当前阶段改为 `daily_bars_local` 本地计算并验收；Tushare 版后置 |
| `broker_forecast` | `report_rc` | 权限确认后作为 stretch |
| `chip_performance` | `cyq_perf` | 权限确认后作为 stretch |
| `chip_distribution` | `cyq_chips` | 权限确认后作为 stretch |

### 执行工作流(WS-0 前置 + 4 条实施线)
...
- **WS-1**:stocks 补全 + 核心事实表时间列标准化 + hypertable 落地 + `fund_flows` 兼容策略 + top_list 替换 + `pro_bar(asset="E")` 统一股票日线行情
- **WS-2**:新事实表按优先级推进；当前阶段以 `technical_factors(local)` 为验收主线，`daily_basic` / `stock_st` 在 token/source contract 就绪前保持 gated
- **WS-3**:完整性检查 + 范围校验 + 跨源一致性 + 告警 + 回填工具 + 时序健康检查
- **WS-4**:Repository + API 端点 + 当前阶段可验收的筛选接入；`daily_basic` / `stock_st` 相关筛选条件在 token/source contract 就绪前保持 gated follow-up

详细重启口径见 [M1_RESTART_PLAN.md](./milestones/m1/M1_RESTART_PLAN.md)。
...
- 当前阶段验收口径不再以 Tushare token 可用性为前提
- 完成后才能作为 `P3-03` / `P3-05` / `P3-06` 的硬前置

### 验收标准
- [ ] Alembic baseline/stamp 策略已明确并验证;当前 `m1/data-layer-restart` 的 M1 revisions 先按 existing-schema migration path 验收
- [ ] 当前阶段纳入验收的新表有 Model + Migration + Fetch Task 或 Backfill Entry + Unit Test
- [ ] stocks 表包含 `stock_basic` 全字段
- [ ] 核心事实表已统一标准时间列
- [ ] `daily_bars` / `fund_flows` / `indicators` / `patterns` 已迁移为 hypertable,并配置 compression 基线
- [ ] `moneyflow` 是否替换 `fund_flows` 已通过显式 migration 决策处理
- [ ] 通用行情接口支持 `asset=E` 股票日线获取
- [ ] 主抓取任务已切换到标准时间列与 `fetch_pro_bar(asset="E")` 口径
- [ ] 数据质量检查覆盖:完整性 + 范围 + 跨源一致性
- [ ] 时序健康检查覆盖:hypertable / chunk / compression / retention / 查询计划
- [ ] 历史数据回填工具可按日期范围运行
- [ ] 至少完成一次 `daily_bars` 回填演练 + 一次 `technical_factors(local)` 回填演练并保留记录
- [ ] 新数据通过 API 端点可访问
- [ ] `daily_basic` / `stock_st` 被明确记录为 gated follow-up，直到有效 Tushare token 或单独批准的 source contract 就绪
- [ ] 基于 `daily_basic` / `stock_st` 的筛选能力不计入当前阶段 M1 验收
...
- [ ] **核心事实表 migration** - `daily_bars` / `fund_flows` / `indicators` / `patterns`
...
**第一优先级: 关闭 `M5 / P3-05` PR/release**
1. 保持当前口径：`M5 / P3-05 v1` 已完成本地代码验收并已提交 PR，但尚未写成生产完成。
2. 复核 PR 合并前证据：本地 focused smoke、frontend typecheck/build、backend full pytest 与 artifact 记录必须能互相对上。
3. 把 live staging smoke、生产 backup、发布前 schema-contract gate 与 release smoke 作为 release activity 收口。
4. 合并/发布完成前，`M6 / P3-06` 只可做准备和设计回看，不作为 active milestone 接棒。

**然后按 active milestone 顺序推进**
5. `M5` `P3-05` PR/release closure
6. `M6` `P3-06` 数据洞察报告（等 `M5` 合并/发布后接棒）
7. `M7` `P3-02` 推荐（依赖行为数据积累,最后收官）

**已退役项**
8. 旧 `M4 / P3-04 策略社交` 与 `PRD v2.0` 的“非社区化”硬边界冲突,仅保留为历史设计参考,不再进入 active plan-of-record
...
- `M2`、`Pre-M3` 与 `M3 / P3-03` 均已完成；`M5 / P3-05 v1` 已完成本地代码验收并已提交 PR，当前短期 active 是 `M5` PR/release closure

### Phase 3 路线顺序
1. **P3-01 用户行为埋点**（P3 底座，已完成）
   - 为推荐、报告、运营分析提供事件基础

2. **Pre-M3 主线对齐修复**（已完成）
   - 收口 release correctness、runtime/schema drift、文档漂移与残留清理
   - 作为 `M3` 的前置 gate 已关闭，evidence 保留在 `docs/milestones/m3/`

3. **参数化指标筛选与订阅提醒**（已完成）
   - 以模板化方式保存并运行带参数的指标筛选器
   - 已完成 `Saved Screener` canonicalization、registry-backed metadata、`RSI / MACD / BOLL` 参数化 runtime 与 Selection 最小接线
   - 已完成 `Alert Subscription`、手动触发、run/hit、站内摘要通知、盘后 scheduler/checker 与 reviewer closure
   - 已完成 post-merge staging 验证与 `0.4.1` 生产收尾；`M3-C` 快捷入口是后续增强项

4. **自动化优化**（当前主线：PR/release closure）
   - 回测参数自动调优（网格搜索/贝叶斯优化）
   - 最佳参数组合推荐
   - 过拟合检测提醒
   - `M5 / P3-05 v1` 本地代码验收已完成并已提交 PR；live staging smoke、生产 backup、发布前 schema-contract gate 与 release smoke 仍是 release activity
...
7. **已退役: `P3-04 策略社交`**
   - 与 `PRD v2.0` 的“不是社区平台”硬边界冲突
   - `phase3/P3-04*` 仅保留为历史设计,不代表承诺实施或 runtime contract

### 任务清单
- [x] **P3-01 用户行为埋点与收集**（执行包与验收见 `docs/milestones/m2/`）
  - 已完成首波事件链路、focused tests、前端接线与 reviewer gate
- [x] **Pre-M3 主线对齐修复**（关闭记录见 `docs/milestones/m3/`）
  - truthful `schema_contract` onboarding / gate 已在真实目标库通过
  - `Strategy` runtime/schema residue 与 active-path data completeness 已收口
  - 主线文档与恢复路径已对齐，reviewer 已给 `go to start M3`
- [x] **P3-03 参数化指标筛选与订阅提醒**（已完成；设计与验收文档见 `docs/milestones/m3/M3_P3-03_ALERT_ENGINE_PLAN.md`）
  - canonical model：`Saved Screener + Alert Subscription + Registry + Adapter`
  - 首批指标：`MACD / RSI / BOLL`
  - 已完成：手动筛选 -> 保存筛选器 canonicalization -> registry-backed metadata -> 订阅提醒 -> 应用内摘要通知
- [-] **P3-05 参数优化服务**（当前主线：PR/release closure；执行入口见 `docs/milestones/m5/README.md`）
  - 首轮最小闭环：创建优化任务 -> 执行有限 trial -> 记录进度和结果 -> 提取最优参数 -> 一键应用到回测配置
  - 运行时约束：先复用现有 `BacktestService` 与当前 async task 模式；不默认引入 Celery/RQ
  - 算法策略：`random_search` 作为 v1 验收目标；贝叶斯优化作为可选增强，只有在接口与测试补齐后进入 runtime
  - 当前状态：`M5 / P3-05 v1` 已完成本地代码验收并已提交 PR；不声称生产完成
  - 剩余 release activity：live staging smoke、生产 backup、发布前 schema-contract gate、release smoke
- [ ] **P3-06 数据洞察报告生成**（设计文档见 `docs/milestones/phase3/PHASE3_P3-06_DATA_INSIGHTS.md`）
  - 三种报告：每日市场晨报 / 每周操作回顾 / 月度策略总结
  - 定时任务推送（08:30/周一09:00/每月1日10:00）
  - 应用内通知 + 邮件（可选） + 偏好设置
  - 报告详情页 + Dashboard 卡片
- [ ] **P3-02 智能推荐服务**（设计文档见 `docs/milestones/phase3/PHASE3_P3-02_RECOMMENDATION.md`）
...
### 前置条件
- **`M0` / `M1` / `M2` 已完成并收口**
- `Pre-M3` reviewer gate 必须为 `go`,否则不启动 `M3`
